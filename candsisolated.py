...
candidate_values = [4,5,6,7]
for c in candidate_values:
    plt.figure(figsize=(10, 6))

    cfiltered_df = df[(df['Candidates'] == c)]
    if c==7:
        ...
        schulze = cfiltered_df[(cfiltered_df['Voting Method'] == 'Schulze')].iloc[:, -7:-(7-c)].mean().reset_index()
        bucklin = cfiltered_df[(cfiltered_df['Voting Method'] == 'Bucklin')].iloc[:, -7:-(7-c)].mean().reset_index()
        plurality_with_runoff = cfiltered_df[(cfiltered_df['Voting Method'] == 'Plurality With Runoff')].iloc[:, -7:-(7-c)].mean().reset_index()
        coombs = cfiltered_df[(cfiltered_df['Voting Method'] == 'Coombs')].iloc[:, -7:-(7-c)].mean().reset_index()
    ...
    plt.title('Percentage True Winner Reached by Ballot Length')
    ...
    # final_df.to_csv(f'{c}_candidates_isolated.csv', index=False)
